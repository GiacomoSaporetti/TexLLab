...
function grid::GetElemS(row,col)
{
	...
	if (col == columns.index || col == columns.subindex)
		// visualizza in hex per index, subindex, cobid
		grid.EventResult = parseInt(s).toString(16)
	else if (col == columns.COBID)
	{
		if ( s === "" || isNaN(s) )
		{
			// se non è un numero proviene da configurazione dinamica del nodo
			// presa dall'attributo COBIDstr
			grid.EventResult = s
		}
		else
		{
			// se è un numero può essere lo converto sempre in esadecimale
			grid.EventResult = parseInt(s).toString(16)
		}
	}
	else
...
function grid::SetElemS(row,col,s)
{
	...
	if (col == columns.numPDO || col == columns.bitstart)
		UpdatePDO()
}
