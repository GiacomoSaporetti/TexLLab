...
{	
	...
	grid.AddColumn( 100, 100, true, false, egColumnType.egEdit,  		0, app.Translate("Parent"));
	grid.AddColumn( 100, 100, true, false, egColumnType.egEdit,  		0, app.Translate("Type"));
	grid.AddColumn( 100, 100, true, false, egColumnType.egEdit,  		0, app.Translate("Address"));
	grid.AddColumn( 250, 100, true, false, egColumnType.egEdit,  		0, app.Translate("Description"));
	...
	if (!res) {
		app.PrintMessage(genfuncs.FormatMsg(app.Translate("Could not load unused variables file \"%1%2\". Enable its generation from the Project Options."), prjName, UNUSED_SUFFIX));
		app.CallFunction("extfunct.SelectOutputTab", TAB_RESOURCES);

		varTypeSel.disabled = true;
...
function GetXMLVars(xmldoc) {
	...
	// carico tutte le uvar
	let nodeList = xmldoc.selectNodes("*//uvar");
	...
	while (node = nodeList.nextNode()) {
		...
		currVar.address = node.getAttribute('addr');
		currVar.type = node.getAttribute('type');
		currVar.description = node.getAttribute('descr');

		let parentNode = node.parentNode;
		if (parentNode.nodeName == "program") {
			currVar.parent = parentNode.getAttribute("name");
			currVar.location = VAR_LOCATION.LOCAL;
		}
		else {
			currVar.parent = node.getAttribute("group");
			currVar.location = VAR_LOCATION.GLOBAL;
		}
...
function LoadGridContents(filter)
{
	...
	for (var i = 0, t = m_globalVars.length; i < t; i++)
	{
		...
		if (varLocationFilter != VAR_LOCATION.ALL && varLocationFilter != plcVar.location)
			continue;
...
function BuildRow(plcVar)
{
	...
	newRow.parent		= plcVar.parent;
	newRow.address		= plcVar.address;
	newRow.type			= plcVar.type;
	newRow.description	= plcVar.description;
	newRow.SID = SID;
...
function FilterVar(plcVar, filter)
{
	if (filter)
	{
		...
		if (filter.findInParent && MatchString(plcVar.parent, filter.searchText, false, filter.wholeWord))
			foundSomethig = true;

		if (filter.findInType && MatchString(plcVar.type, filter.searchText, false, filter.wholeWord))
			foundSomethig = true;

		if (filter.findInAddress && MatchString(plcVar.address, filter.searchText, false, filter.wholeWord))
			foundSomethig = true;
...
function FilterParameters()
{
	...
	app.OpenWindow("LLUtils_FindUnusedVarsReport", app.Translate("Search filters"), "");
...
function PrintWindow()
{
	...
	for (let index in m_gridRows) {
		...
		nodeNew.setAttribute("parent", rowData.parent);
		nodeNew.setAttribute("type", rowData.type);
		nodeNew.setAttribute("address", rowData.address);
		nodeNew.setAttribute("description", rowData.description);
	...
	app.OpenWindow("LLUtils_PrintUnusedVarsReport", app.Translate("Print preview"), "")
	return true
...
function DeleteSelected()
{
	...
	var ris = app.MessageBox(app.Translate("Do you want to delete the selected variables?"), "", gentypes.MSGBOX.MB_ICONQUESTION|gentypes.MSGBOX.MB_YESNO);
	...
	for (var i = 0; i < selections.length; i++)
	{
		...
		if(app.CallFunction("logiclab.DeleteObject", sid))
			...
			app.PrintMessage(genfuncs.FormatMsg(app.Translate("Could not delete '%1' variable"), name));
			app.CallFunction("extfunct.SelectOutputTab", TAB_RESOURCES);
		}
