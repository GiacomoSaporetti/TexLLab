...
function drawData(treeData) {
  ...
  var svg = d3.select("#treeView").append("svg").attr("width", width - margin.left - margin.right).attr("height", destHeight - margin.top - margin.bottom),
  ...
  node.on("click", function (el) {
    m_clickNum++;
    setTimeout(function () {
      var SID = el.data._SID;
      if (m_clickNum == 1) {
        // seleziona la POU cliccata
        if (SID) NodeClick(SID);
      } else if (m_clickNum == 2) {
        // naviga alla definizione
        if (SID) app.CallFunction("logiclab.GoToPLCLink", SID);
      }
      m_clickNum = 0;
    }, 300);
  });
